...
private:
    multimap<string, unique_ptr<Book>> books = multimap<string, unique_ptr<Book>>();
...
public:
    ...
    void readBookHistory(shared_ptr<ostream> bookDatabase,
...
public:
    GetRecommendationCommand(shared_ptr<BookStorage> _bs)
...
public:
    // 수정함
    virtual const char* what() const noexcept { return "User already exsits\n"; }
};

class DatabaseNotOpen : public exception {
public:
    // 수정함
    virtual const char* what() const noexcept { return "File not opened\n"; }
};
...
public:
    ...
    shared_ptr<BookManager> getBookManager() { return bookManager; }
...
public:
    ...
    multimap<string, shared_ptr<Book>> getReadBookHistory() {
        return readBookHistory;
    }
    multimap<string, shared_ptr<Book>> getPurchaseBookHistory() {
        return purchaseBookHistory;
    }
    shared_ptr<BookRecommender> getBookRecommender() {
